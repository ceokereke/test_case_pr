...
def CS (test_cases, no_priority_cases, MaxT = 100, pa = 0.25):
    ...
    if ld == 0:
        ...
        with open('load_ft.pkl', 'wb') as outp1:
            pick.dump(fitness, outp1, pick.HIGHEST_PROTOCOL)
    else:
        with open('load_ft.pkl', 'rb') as inp1:
            fitness = pick.load(inp1)
    ...
    for i in range(MaxT):
        ...
        new_fitness = objective_fun(new_population[new_nest],pname)
