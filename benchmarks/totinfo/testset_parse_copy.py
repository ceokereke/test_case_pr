...
def parse(pname,location):
	...
	subprocess.call("gcc -fprofile-arcs -ftest-coverage -g -o "+pname+" "+location+pname+".c  -lm",shell=True)
	...
	for key in testset:
		...
		for line in Sout.readlines():
			...
			if ls[0]=="branch" and ls[2]=="taken" and int(ls[3])>0:
				bcount+=1
			if ls[0]=="call" and ls[2]=="never":
				cncount+=1
			if ls[0]=="call" and ls[2]=="returned":
				crcount+=1
		...
		testset[key].append(cncount)
		testset[key].append(crcount)

	# print(testset)



	with open('output_file_or.txt', 'w') as f:
