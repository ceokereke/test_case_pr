...
def helper(tlist,pname,location,maxlim,option):
	...
	for test in tlist:
		...
		print (temp_out)
		if (option==1) and float(maxlim[pname])==float(temp_out.split(b'\n')[option].split(b':')[-1].split()[0].strip(b'%')):
			return ade
		if (option==2) and 100==float(temp_out.split(b'\n')[option].split(b':')[-1].split()[0].strip(b'%')):
			return ade
		elif (option==3) and (float(maxlim[pname])==float(temp_out.split(b'\n')[1].split(b':')[-1].split()[0].strip(b'%'))) and (100==float(temp_out.split(b'\n')[2].split(b':')[-1].split()[0].strip(b'%'))):
			return ade
